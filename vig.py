...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.edge_index = Grapher(channels, num_knn[0], 1, conv, act, norm,
                                              bias, stochastic, epsilon, 1, drop_path=dpr[0])
        self.model_init()
    ...
    def get_edge_index(self, inputs):
        with torch.no_grad():
            x = self.stem(inputs) + self.pos_embed
            edge_index = self.edge_index.get_edge_index(x).detach().cpu()
        return edge_index
